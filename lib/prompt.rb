...
module Prompt
  ...
  def player_config_prompt
    print player_config_banner.blue
    prompt = TTY::Prompt.new
    # Will change later to address different player combinations
    player = prompt.collect do
      ...
      key(:letter).select("Choose your destiny?".bold.cyan, %w(X O), active_color: :bold)
    end
    # Sends player info to types of players
    clear_terminal
    return set_player_one(player)
  end
